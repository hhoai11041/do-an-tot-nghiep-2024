...
const ReviewDestination = ({ slug, destinationId, setRender, render }) => {
  ...
  const handleSubmitReview = async () => {
    ...
    UpdateApi.updateDestinationReviewById(
      accessToken,
      destinationId,
      newTotalStars,
  ...
  return (
    <div className="w-full mx-auto">
      <h1 className="text-center screenLarge:text-2xl desktop:text-2xl laptop:text-2xl tablet:text-xl mobile:text-xl font-bold uppercase text-textPrimary">
        Đánh giá gần đây
      </h1>

      {loading ? (
        <div className="relative h-[100px] w-full mt-10">
          {loadingApp.loadingCircle(
            "absolute top-1/2 left-[50%] translate-y-[-50%] translate-x-[-50%]"
          )}
        </div>
      ) : (
        <div>
          <div className="bg-[#fef0ed] rounded-lg py-4 my-4 screenLarge:flex desktop:flex laptop:flex">
            <div className="screenLarge:w-[25%] desktop:w-[25%] laptop:w-[25%] tablet:w-[25%] flex flex-col justify-center text-center text-[22px] font-bold text-red-600 mb-4">
              <span className="">
                <span className="text-[35px]">{totalStars?.toFixed(1)}</span>{" "}
                trên 5.0
              </span>
          ...
          {updatedReviews &&
            updatedReviews?.map((item, index) => (
              <div className="py-2" key={index}>
                <div className=" border-b-2 dark:border-gray-700 dark:border-b-[1px]">
                  <div className="flex justify-start gap-3">
                    ...
                    <div className="text-gray-700 dark:text-textThemeUI font-semibold">
      ...
      <ModalAdmin
        ...
        className="screenLarge:w-[50%] desktop:w-[60%] laptop:w-[60%] tablet:w-[80%] mobile:w-[95%] screenLarge:h-auto desktop:h-[90vh] laptop:h-auto tablet:h-[80vh] mobile:h-[80vh] overflow-y-scroll bg-white px-7 py-10 rounded-sm reviewModal"
      />
    </div>
  );
};
